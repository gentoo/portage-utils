...
)
{
  ...
  char           *xpak;
  enum {
    XPAK_ACT_NONE, XPAK_ACT_LIST, XPAK_ACT_EXTRACT, XPAK_ACT_CREATE
  }               action = XPAK_ACT_NONE;
  int             i;
  int             ret;
  ...
  if (argc > 0 &&
      (action == XPAK_ACT_LIST ||
       action == XPAK_ACT_EXTRACT))
    for (i = 0; i < argc; i++)
      if (argv[i])
