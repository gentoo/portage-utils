...
static gpg_sig *verify_gpg_sig(const char *path, verify_msg **msgs);
static char    *verify_timestamp(const char *ts);
 ...
 * list.  The list is returned as array, which needs to be deep-freed.
 */
static array *
list_dir
(
...
)
{
  ...
  if ((d = opendir(path)) != NULL)
  {
    ...
    while ((e = readdir(d)) != NULL)
    {
      ...
      array_append_strcpy(list, e->d_name);
    }
    closedir(d);

    array_sort(list, NULL);
  }
...
)
{
  ...
  if (hashes & HASH_BLAKE2B)
    len += snprintf(data + len, sizeof(data) - len, " BLAKE2B %s", blak2b);
  if (hashes & HASH_SHA256)
    len += snprintf(data + len, sizeof(data) - len, " SHA256 %s", sha256);
  if (hashes & HASH_SHA512)
    len += snprintf(data + len, sizeof(data) - len, " SHA512 %s", sha512);
  len += snprintf(data + len, sizeof(data) - len, "\n");

  if (m != NULL)
    fwrite(data, len, 1, m);
  if (gm != NULL &&
      gzwrite(gm, data, len) == 0)
    err("failed to write to compressed stream!");
}
...
)
{
  ...
  if (dirs != NULL)
  {
    array_for_each(dirs, i, dentry)
    {
      snprintf(path, sizeof(path), "%s/%s", name, dentry);
        ...
        do
        {
          ...
          do
          {
            ...
            if (strncmp(tok, "SHA256", sz) == 0)
              ...
              warn("unsupported hash from layout.conf: %.*s", (int)sz, tok);
...
)
{
  ...
  if (mtype == GLOBAL_MANIFEST)
  {
    ...
    if ((mf = gzopen(path, "wb9")) == NULL)
      errp("failed to open file '%s' for writing", path);
    ...
    {
      gzclose(mf);
      errp("failed to write to file '%s/%s'", dir, str_manifest_files_gz);
    }

    if ((dirs = list_dir(dir)) == NULL)
    {
      gzclose(mf);
      return NULL;
    }
    ...
    array_for_each(dirs, i, dentry)
    {
      ...
      if (strcmp(dentry, str_manifest_files_gz) == 0 ||
          strcmp(dentry, str_manifest) == 0)
        continue;

      snprintf(path, sizeof(path), "%s/%s", dir, dentry);
      ...
      if (stat(path, &s) == 0)
      {
        ...
        {
          if (strcmp(dentry, "eclass")   == 0 ||
              strcmp(dentry, "licenses") == 0 ||
              strcmp(dentry, "metadata") == 0 ||
              strcmp(dentry, "profiles") == 0 ||
              strcmp(dentry, "scripts")  == 0)
          ...
          if (mfest == NULL)
          {
            warn("generating Manifest for %s failed!", path);
            gzclose(mf);
            array_deepfree(dirs, NULL);
            return NULL;
          }

          snprintf(path, sizeof(path), "%s/%s", dentry, mfest);
          write_hashes(tv, dir, path, "MANIFEST", NULL, mf);
        }
        else if (s.st_mode & S_IFREG)
        {
          write_hashes(tv, dir, dentry, "DATA", NULL, mf);
        } /* ignore other "things" (like symlinks) as they
             don't belong in a tree */
      }
      else
      {
        warnp("stat(%s) failed", path);
      }
    }
    array_deepfree(dirs, NULL);
    gzclose(mf);
  ...
  else if (mtype == SUBTREE_MANIFEST)
  {
    ...
    if (strcmp(ldir, "metadata") == 0)
    {
      ...
      if (gzwrite(mf, path, len) == 0)
      {
        warnp("failed to write to file '%s/%s'", dir, str_manifest_gz);
        gzclose(mf);
        return NULL;
      }
    }
    ...
    {
      /* ignore existing Manifests */
      if (strcmp(dentry, str_manifest_gz) == 0)
        continue;

      if (write_hashes_dir(tv, dir, dentry, mf) != 0)
        write_hashes(tv, dir, dentry, "DATA", NULL, mf);
    }
  ...
  else if (mtype == EBUILD_MANIFEST)
  {
    ...
    if (f != NULL)
    {
      ...
      while (fgets(path, sizeof(path), f) != NULL)
      {
        if (strncmp(path, "DIST ", 5) == 0)
        {
          ...
          {
            warnp("failed to write to %s/.Manifest.new", dir);
            fclose(f);
            fclose(m);
            return NULL;
          }
    ...
    if ((dirs = list_dir(dir)) != NULL)
    {
      array_for_each(dirs, i, dentry)
      {
        if (strcmp(dentry + strlen(dentry) - 7, ".ebuild") != 0)
          continue;
        write_hashes(tv, dir, dentry, "EBUILD", m, NULL);
      }
      array_deepfree(dirs, NULL);
    }
    ...
    if (rename(newmanifest, path) == -1)
      errp("failed to rename file '%s' to '%s'", newmanifest, path);
...
)
{
  ...
  if (array_cnt(elems) == 0)
    return ret;  /* nothing to do */

  /* shortcut a single Manifest entry pointing to the same dir
   * (happens at top-level) */
  entry = array_get(elems, 0);
  if (array_cnt(elems) == 1 &&
      skippath == 0 &&
      entry[0] == 'M' &&
      strchr(&entry[2], '/') == NULL)
  {
    if ((ret = verify_file(dir, &entry[2], mfest, msgs)) == 0)
    {
      slash = strchr(&entry[2], ' ');
      if (slash != NULL)
        *slash = '\0';
      /* else, verify_manifest will fail, so ret will be handled */
      ret = verify_manifest(dir, &entry[2], msgs);
    }
    return ret;
  }
  ...
  if ((dirs = list_dir(dir)) != NULL)
  {
    ...
    while (curdentry < array_cnt(dirs))
    {
      dentry = array_get(dirs, curdentry);

      if (strcmp(dentry, str_manifest) == 0 ||
          strcmp(dentry, str_manifest_gz) == 0 ||
          strcmp(dentry, str_manifest_files_gz) == 0)
      {
        curdentry++;
        continue;
      }

      if (curelem < array_cnt(elems))
      {
        entry  = array_get(elems, curelem);
        etpe   = entry[0];
        entry += 2 + skippath;
      }
      else
      ...
      if (checkdir &&
          (slash = strchr(entry, '/')) != NULL)
      {
        char  *sentry;
        size_t sublen = slash - entry;

        subdir = array_new();
        array_append(subdirs, subdir);

        /* collect all entries like this one (same subdir) into
         * a sub-list that we can verify */
        while ((sentry = array_get(elems, curelem)) != NULL &&
               strncmp(entry, sentry + 2 + skippath, sublen + 1) == 0)
        {
          array_append(subdir, sentry);
          curelem++;
        }

        curelem--;         /* move back, see below */
        checkdir = false;  /* avoid re-check if we don't move forward */
        sentry   = array_get(elems, curelem);

        /* modify the last entry to be the subdir, such that we
         * can let the code below synchronise with dentries */
        sentry[2 + skippath + sublen] = ' ';
        entry = sentry + 2 + skippath;
    ...
    curelem = array_cnt(subdirs);
#pragma omp parallel for shared(ret) private(entry, etpe, slash, subdir)
    for (elem = 0; elem < curelem; elem++)
    { /* OpenMP doesn't like array_for_each */
      char   ndir[_Q_PATH_MAX];
      char  *p;
      size_t subdirlen;

      subdir    = array_get(subdirs, elem);

      /* restore original entry format */
      entry     = array_get(subdir, array_cnt(subdir) - 1);
      p         = strchr(entry + 2 + skippath, ' ');
      if (p == NULL)
        continue;  /* impossible */
      *p        = '/';
      subdirlen = p - (entry + 2 + skippath);

      entry     = array_get(subdir, 0);
      etpe      = entry[0];
      entry    += 2 + skippath;

      if (etpe == 'M')
      {
        size_t skiplen = strlen(dir) + 1 + subdirlen;
      ...
      else
      {
        snprintf(ndir, sizeof(ndir), "%s/%.*s", dir,
                 (int)subdirlen, entry);
        ret |= verify_dir(ndir, subdir, skippath + subdirlen + 1, mfest, msgs);
      }
...
#define append_list(STR) \
  {\
    ...
    {\
      ...
      {\
        STR[5] = 'I';\
        array_append_strcpy(elems, &STR[5]);\
      }\
      else if (strncmp(STR, "MANIFEST ", 9) == 0)\
      {\
        STR[7] = 'M';\
        array_append_strcpy(elems, &STR[7]);\
      }\
      ...
      {\
        ...
        {\
          STR[3] = 'D';\
          array_append_strcpy(elems, &STR[3]);\
        }\
      }\
      else if (strncmp(STR, "AUX ", 4) == 0)\
      {\
        /* translate directly into what it is: DATA in files/ */\
        array_append(elems, xasprintf("D files/%s", &STR[4]));\
      }\
    }\
  }
   ...
   */
  array_sort(elems, compare_elems);
  snprintf(buf, sizeof(buf), "%s/%s", dir, manifest);
  ret = verify_dir(dir, elems, 0, buf + 2, msgs);
  checked_manifests++;

  array_deepfree(elems, NULL);
...
)
{
  ...
  if (runfunc == NULL)
    runfunc = process_dir_vrfy;  /* default mode: verify */
