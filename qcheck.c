...
#define qcprintf(fmt, args...) do { if (!qcheck_state.bad_only) printf(_(fmt), ## args); } while (0)
...
/* Define globally, enabling broader qcprintf() usage */
static struct qcheck_opt_state qcheck_state;

static int
qcheck_process_contents(q_vdb_pkg_ctx *pkg_ctx)
{
	int fd_contents;
	...
	qcprintf("%sing %s%s/%s%s ...\n",
		(qcheck_state.qc_update ? "Updat" : "Check"),
		GREEN, catname, pkgname, NORM);
	if (qcheck_state.qc_update) {
	...
	if (!qcheck_state.chk_config_protect) {
	...
	while (getline(&line, &linelen, fp_contents) != -1) {
		...
		/* run initial checks */
		++num_files;
		if (array_cnt(qcheck_state.regex_arr)) {
			size_t n;
			regex_t *regex;
			array_for_each(qcheck_state.regex_arr, n, regex)
				if (!regexec(regex, entry->name, 0, NULL, 0))
					break;
			if (n < array_cnt(qcheck_state.regex_arr)) {
				--num_files;
				++num_files_ignored;
				continue;
			}
		...
		if (!qcheck_state.chk_mtime && entry->type == CONTENTS_SYM) {
			--num_files;
			++num_files_ignored;
			if (qcheck_state.qc_update)
				fputs(buffer, fp_contents_update);
		...
		if (entry->digest && S_ISREG(st.st_mode)) {
			...
			hash_cb_t hash_cb = qcheck_state.undo_prelink ? hash_cb_prelink_undo : hash_cb_default;
		...
		/* Validate mtimes */
		if (qcheck_state.chk_mtime && entry->mtime && entry->mtime != st.st_mtime) {
	...
	if (qcheck_state.bad_only && num_files_ok != num_files) {
		if (verbose)
			printf("%s/%s\n", catname, pkgname);
		else {
...
qcheck_cb(q_vdb_pkg_ctx *pkg_ctx, void *priv)
{
	...
	if (array_cnt(qcheck_state.atoms)) {
		char *buf;
		size_t i;
		...
		array_for_each(qcheck_state.atoms, i, atom)
	...
	return showit ? qcheck_process_contents(pkg_ctx) : 0;
}
...
int qcheck_main(int argc, char **argv)
{
	...
	qcheck_state.atoms = atoms;
	qcheck_state.regex_arr = regex_arr;
	qcheck_state.bad_only = false;
	qcheck_state.qc_update = false;
	qcheck_state.chk_afk = true;
	qcheck_state.chk_hash = true;
	qcheck_state.chk_mtime = true;
	qcheck_state.chk_config_protect = true;
	qcheck_state.undo_prelink = false;
	...
	while ((ret = GETOPT_LONG(QCHECK, qcheck, "")) != -1) {
		...
		case 'u': qcheck_state.qc_update = true; break;
		case 'A': qcheck_state.chk_afk = false; break;
		case 'B': qcheck_state.bad_only = true; break;
		case 'H': qcheck_state.chk_hash = false; break;
		case 'T': qcheck_state.chk_mtime = false; break;
		case 'P': qcheck_state.chk_config_protect = false; break;
		case 'p': qcheck_state.undo_prelink = prelink_available(); break;
		}
	}
	...
	ret = q_vdb_foreach_pkg_sorted(qcheck_cb, &qcheck_state);
	{
		void *regex;
