  ...
  do
  {
    ...
    *q = '\0';
    ...
    if (match)
    {
      ...
      if (state->match == NULL ||
          atom_compare_flg(atom, state->match, (ATOM_COMP_NOSLOT |
  ...
  struct quse_state *state)
{
  ...
  while ((de = readdir(d)))
  {
    ...
    if (namelen <= 5 ||
        strcmp(de->d_name + namelen - 5, ".desc") != 0)
    {
      ret = false;
      break;
    }
