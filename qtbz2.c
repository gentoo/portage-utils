...
)
{
  ...
  if (pread(fd, buf, 8, 0) != 8 ||
      memcmp(buf, "XPAKPACK", 8))
...
)
{
  ...
  char *tarbz2;
  enum {
    TBZ2_ACT_NONE, TBZ2_ACT_JOIN, TBZ2_ACT_SPLIT
  }     action;
  int   i;
  int   dir_fd;
  bool  split_xpak = true;
  bool  split_tarbz2 = true;
  ...
  while ((i = GETOPT_LONG(QTBZ2, qtbz2, "")) != -1)
  {
    ...
    case 't': action = TBZ2_ACT_SPLIT;
              split_xpak = false;       break;
    case 'x': action = TBZ2_ACT_SPLIT;
              split_tarbz2 = false;     break;
    case 'O': tbz2_stdout = true;       break;
    case 'd':
      if (dir_fd != AT_FDCWD)
        close(dir_fd);
